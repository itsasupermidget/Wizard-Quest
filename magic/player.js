...
function swingSword(parent) {
  if (player.inventory.includes("sword") && !player.attacks.includes("sword")) {
...
function mistAttack(parent) {
  if (parent.mana >= 10 && (!parent.animation || (parent.animation.name != "attack" || parent.animation.current > 4))) {
    parent.charging = false;
    if (parent.facing == 1) {
